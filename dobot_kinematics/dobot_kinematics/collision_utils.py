...
class CollisionDetector:
    ...
    def compute_distances(self, q, max_distance=1.0):
        ...
        for i in range(4):
            pyb.resetJointState(
                self.robot_id, i+1, q[i], physicsClientId = self.col_id
            )
        ...
        for a, b in self.indexed_collision_pairs:
            ...
            if len(closest_points) == 0:
                ...
                distances.append(np.min([pt[8] for pt in closest_points])) # contact distance, positive for separation, negative for penetration

        return np.array(distances)


    def print_collision_pairs(self, d):
        names = []
        idx = 0
        for a, b in self.indexed_collision_pairs:
                pair = []
                if a.body_uid == 0:
                    first = 'robot end effector'
                    if b.body_uid == 0 and (b.link_uid == 0 or b.link_uid == 1):
                        second = 'robot base'
                    else:
                        second = list(self.bodies.keys())[b.body_uid]
                    if d[idx] > 0:
                        pair = [first, second, "SAFE"]
                    else:
                        pair = [first, second, "IN COLLISION"]
                    idx = idx + 1
                names.append(pair) 
