...
class GripperService(Node):
    ...
    def gripper_callback(self, request, response):
        ...
        if request.gripper_state == "open" and request.keep_compressor_running == True:
            manipulators[self.get_namespace()].set_end_effector_gripper(True, False)
            ...
            manipulators[self.get_namespace()].set_end_effector_gripper(True, False)
            time.sleep(self.gripper_latency)
            manipulators[self.get_namespace()].set_end_effector_gripper(False, False)
            msg.status = "opened"
            self.publisher_.publish(msg)
        elif request.gripper_state == "close" and request.keep_compressor_running == True:
            manipulators[self.get_namespace()].set_end_effector_gripper(True, True)
            time.sleep(self.gripper_latency)
            msg.status = "closed"
            self.publisher_.publish(msg)
            ...
            manipulators[self.get_namespace()].set_end_effector_gripper(True, True)
            time.sleep(self.gripper_latency)
            manipulators[self.get_namespace()].set_end_effector_gripper(False, True)
            msg.status = "closed"
            self.publisher_.publish(msg)
        else:
