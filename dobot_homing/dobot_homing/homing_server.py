...
class HomingService(Node):
    ...
    def parameters_callback(self, params):
        ...
        for param in params:
            if param.name == 'homing_position' and param.type_ == Parameter.Type.DOUBLE_ARRAY:
                manipulators[self.get_namespace()].set_homing_parameters(param.value[0], param.value[1], param.value[2], param.value[3])
    ...
    def homing_callback(self, request, response):
        ...
        manipulators[self.get_namespace()].set_homing_command(0) 
        time.sleep(0.1)
