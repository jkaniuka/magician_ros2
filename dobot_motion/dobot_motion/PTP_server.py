...
class DobotPTPServer(Node):
    ...
    def send_joint_parameters(self):
            ...
            manipulators[self.get_namespace()].set_point_to_point_joint_params(params[0:4], params[4:8])

    def send_cartesian_parameters(self):
            params = self.sort_params_dict(self.cartesian_params_dict, "cartesian")
            manipulators[self.get_namespace()].set_point_to_point_coordinate_params(params[0], params[1], params[2], params[3])
    ...
    def goal_callback(self, goal_request):
        ...
        if DobotPTPServer.is_ratio_valid(goal_request.velocity_ratio) and DobotPTPServer.is_ratio_valid(goal_request.acceleration_ratio):
            ...
            manipulators[self.get_namespace()].set_point_to_point_common_params(vel_ratio, acc_ratio)
        else:
    ...
    async def execute_callback(self, goal_handle):
        ...
        manipulators[self.get_namespace()].set_point_to_point_command(self.motion_type, self.target[0], self.target[1], self.target[2], self.target[3])
