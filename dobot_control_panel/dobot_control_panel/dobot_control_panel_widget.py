...
class DobotControlPanel(QWidget):
    ...
    def __init__(self, node, plugin=None):
        ...
        if os.environ.get('MAGICIAN_RAIL_IN_USE') == "true":
            ...
            manipulators[self._node.get_namespace()].set_sliding_rail_status(1,1)
            manipulators[self._node.get_namespace()].set_point_to_point_sliding_rail_params(100, 100)
            manipulators[self._node.get_namespace()].set_point_to_point_sliding_rail_params(100*2, 100*2) #BUG
        ...
        # initial setup
        manipulators[self._node.get_namespace()].set_jog_common_params(100, 1) # (vel_ratio, acc_ratio)
        manipulators[self._node.get_namespace()].set_jog_joint_params([100, 100, 100, 100], [100, 100, 100, 100])
        manipulators[self._node.get_namespace()].set_jog_coordinate_params([100, 100, 100, 100], [100, 100, 100, 100])

        # print namespace
        self.NamespacePrint.setText(str(self._node.get_namespace()))
    ...
    def JT_IDLE(self):
        if self.frame == "base":
            manipulators[self._node.get_namespace()].set_jog_command(0, 0)
        elif self.frame == "joint":
            manipulators[self._node.get_namespace()].set_jog_command(1, 0)

        
    def JT1_move(self, sign):
        if sign.text() == "+":
            if self.frame == "base":
                manipulators[self._node.get_namespace()].set_jog_command(0, 1)
            elif self.frame == "joint":
                manipulators[self._node.get_namespace()].set_jog_command(1, 1)
        elif sign.text() == "-":
            if self.frame == "base":
                manipulators[self._node.get_namespace()].set_jog_command(0, 2)
            elif self.frame == "joint":
                manipulators[self._node.get_namespace()].set_jog_command(1, 2)

    def JT2_move(self, sign):
        if sign.text() == "+":
            if self.frame == "base":
                manipulators[self._node.get_namespace()].set_jog_command(0, 3)
            elif self.frame == "joint":
                manipulators[self._node.get_namespace()].set_jog_command(1, 3)
        elif sign.text() == "-":
            if self.frame == "base":
                manipulators[self._node.get_namespace()].set_jog_command(0, 4)
            elif self.frame == "joint":
                manipulators[self._node.get_namespace()].set_jog_command(1, 4)

    def JT3_move(self, sign):
        if sign.text() == "+":
            if self.frame == "base":
                manipulators[self._node.get_namespace()].set_jog_command(0, 5)
            elif self.frame == "joint":
                manipulators[self._node.get_namespace()].set_jog_command(1, 5)
        elif sign.text() == "-":
            if self.frame == "base":
                manipulators[self._node.get_namespace()].set_jog_command(0, 6)
            elif self.frame == "joint":
                manipulators[self._node.get_namespace()].set_jog_command(1, 6)

    def JT4_move(self, sign):
        if sign.text() == "+":
            if self.frame == "base":
                manipulators[self._node.get_namespace()].set_jog_command(0, 7)
            elif self.frame == "joint":
                manipulators[self._node.get_namespace()].set_jog_command(1, 7)
        elif sign.text() == "-":
            if self.frame == "base":
                manipulators[self._node.get_namespace()].set_jog_command(0, 8)
            elif self.frame == "joint":
                manipulators[self._node.get_namespace()].set_jog_command(1, 8)
    ...
    def button_clicked_HomingButton(self):   
        ...
        command = 'ros2 service call ' + self._node.get_namespace() + '/dobot_homing_service dobot_msgs/srv/ExecuteHomingProcedure'
        subprocess.Popen(
                    command, universal_newlines=True, shell=True,
                    stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()

    def button_clicked_EStopButton(self):
        manipulators[self._node.get_namespace()].stop_queue(force=True) 

        command = 'ros2 service call ' + self._node.get_namespace() + '/PTP_action/_action/cancel_goal action_msgs/srv/CancelGoal'
        subprocess.Popen(
                    command, universal_newlines=True, shell=True,
                    stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()
    ...
    def open_gripper(self):
        command = 'ros2 service call ' + self._node.get_namespace() + '/dobot_gripper_service dobot_msgs/srv/GripperControl "{gripper_state: "open", keep_compressor_running: false}"'
        subprocess.Popen(
                    command, universal_newlines=True, shell=True,
                    stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()
    ...
    def close_gripper(self):
        command = 'ros2 service call ' + self._node.get_namespace() + '/dobot_gripper_service dobot_msgs/srv/GripperControl "{gripper_state: "close", keep_compressor_running: false}"'
        subprocess.Popen(
                    command, universal_newlines=True, shell=True,
                    stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()
    ...
    def turn_on_suction_cup(self):
        command = 'ros2 service call ' + self._node.get_namespace() + '/dobot_suction_cup_service dobot_msgs/srv/SuctionCupControl "{enable_suction: true}"'
        subprocess.Popen(
            command, universal_newlines=True, shell=True,
            stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()

    def turn_off_suction_cup(self):
        command = 'ros2 service call ' + self._node.get_namespace() + '/dobot_suction_cup_service dobot_msgs/srv/SuctionCupControl "{enable_suction: false}"'
        subprocess.Popen(
            command, universal_newlines=True, shell=True,
            stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()
    ...
    def change_vel_joints(self, slider):
        manipulators[self._node.get_namespace()].set_jog_joint_params([int(self.Joint1VelSlider.value()), int(self.Joint2VelSlider.value()), int(self.Joint3VelSlider.value()), int(self.Joint4VelSlider.value())], [100, 100, 100, 100])


    def change_vel_cartesian(self, slider):
        manipulators[self._node.get_namespace()].set_jog_coordinate_params([int(self.XVelSlider.value()), int(self.YVelSlider.value()), int(self.ZVelSlider.value()), int(self.RVelSlider.value())], [100, 100, 100, 100])
    ...
    def sliding_rail_SV_slider(self):
        ...
        manipulators[self._node.get_namespace()].set_point_to_point_sliding_rail_command(4,self.dobot_current_joint_states[0], self.dobot_current_joint_states[1], self.dobot_current_joint_states[2], self.dobot_current_joint_states[3], int(self.RailSVSlider.value()))
    ...
    def sliding_rail_params_slider(self):
        if not self.RAIL_IN_USE:
            self.sliding_rail_disconnected()
            return
        manipulators[self._node.get_namespace()].set_point_to_point_sliding_rail_params(int(self.RailVelocitySlider.value()) * 2, int(self.RailAccelerationSlider.value()) * 2) #BUG
    ...
    def button_clicked_StopRail(self):
        if not self.RAIL_IN_USE:
            ...
            return
        manipulators[self._node.get_namespace()].stop_queue(force=True) 
        manipulators[self._node.get_namespace()].clear_queue()
        manipulators[self._node.get_namespace()].start_queue()
